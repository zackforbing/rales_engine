...
# self.joins(invoices: [:invoice_items]).merge( Invoice.successful ).group("merchants.id, invoice_items.id").select("merchants.*, invoice_items.*, sum(invoice_items.quantity * invoice_items.unit_price) AS total_revenue").where("invoices.created_at = '#{date}'").total_revenue
